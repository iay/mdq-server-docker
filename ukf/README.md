...
`service/mdq-server-ukf.conf` is for `upstart`-based systems such as Ubuntu 14.04 LTS.
...
Install the service configuration as follows:
    ...
    # cp service/mdq-server-ukf.conf /etc/init
    # initctl start mdq-server-ukf

### `systemd` Integration

`service/mdq-server-ukf` is for `systemd`-based systems. It is at present untested.
